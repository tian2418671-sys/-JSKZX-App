 ...
 */
const { app, BrowserWindow, ipcMain, dialog, protocol, net, shell, session, nativeImage } = require('electron');
...
// ================= [ 🖼️ 换卡图 + 校验校准：PNG 重建与内嵌工具 ] =================

// 判断图片头部格式（png / jpeg / webp / unknown）
...
// 把任意图片 Buffer（PNG/JPEG/WebP）转成 PNG Buffer；PNG 原样返回，转换失败返回 null
function imageBufferToPng(buffer) {
  if (!buffer) return null;
  if (sniffImageType(buffer) === 'png') return buffer;
  try {
...
// 读取 PNG 的 IHDR 宽高（失败返回 null）
function readPngDimensions(pngBuffer) {
  try {
    if (!pngBuffer || pngBuffer.length < 24 || pngBuffer.readUInt32BE(0) !== 0x89504e47) return null;
...
// 尺寸超限时等比缩放到 maxDim 内；无需缩放或失败时原样返回
function downscalePngIfNeeded(pngBuffer, maxDim) {
  const dims = readPngDimensions(pngBuffer);
  if (!dims) return pngBuffer;
  const max = Math.max(dims.width, dims.height, 0);
  if (max <= maxDim) return pngBuffer;
  try {
    const scale = maxDim / max;
    const img = nativeImage.createFromBuffer(pngBuffer);
    if (!img || img.isEmpty()) return pngBuffer;
    ...
    return (out && out.length) ? out : pngBuffer;
  } catch (e) {
    return pngBuffer;
  }
}

// 构造一个 tEXt 数据块（keyword \0 base64Text）
function buildPngTextChunk(keyword, base64Text) {
  return {
    type: 'tEXt',
    data: Buffer.concat([
      Buffer.from(keyword, 'latin1'),
      Buffer.from([0]),
      Buffer.from(base64Text, 'latin1')
    ])
  };
}

// 根据卡片 spec 选择内嵌关键字：V3 用 ccv3，其余用 chara
function cardKeyword(cardJson) {
  return (cardJson && String(cardJson.spec).toLowerCase() === 'chara_card_v3') ? 'ccv3' : 'chara';
}

// 把卡片 JSON 内嵌进 PNG（去除旧的 chara/ccv3 文本块，重新嵌入，重算 CRC，确保 IEND）
function embedCardJsonInPng(pngBuffer, cardJson, keyword) {
  if (!pngBuffer || pngBuffer.length < 8 || pngBuffer.readUInt32BE(0) !== 0x89504e47) return null;
  const base64Text = Buffer.from(JSON.stringify(cardJson), 'utf-8').toString('base64');
  const sig = pngBuffer.subarray(0, 8);
  const textChunk = buildPngTextChunk(keyword, base64Text);
  ...
  while (offset + 12 <= pngBuffer.length) {
    ...
    const type = pngBuffer.subarray(offset + 4, offset + 8).toString('latin1');
    const data = pngBuffer.subarray(offset + 8, offset + 8 + length);

    // 丢弃旧的 chara/ccv3 文本块（统一用新数据替换，避免幽灵数据残留）
    if (type === 'tEXt' || type === 'iTXt') {
      const nullPos = data.indexOf(0);
      if (nullPos > 0) {
        const kw = data.subarray(0, nullPos).toString('latin1');
        if (kw === 'chara' || kw === 'ccv3') { offset += 12 + length; continue; }
      }
    }
  ...
  if (!inserted) chunks.push(textChunk);
  if (chunks.length === 0 || chunks[chunks.length - 1].type !== 'IEND') {
    chunks.push({ type: 'IEND', data: Buffer.alloc(0) });
  }
  ...
  const parts = [sig];
  for (const chunk of chunks) {
    const tBuf = Buffer.from(chunk.type, 'latin1');
    const lenBuf = Buffer.alloc(4);
    lenBuf.writeUInt32BE(chunk.data.length);
    const crcBuf = Buffer.alloc(4);
    crcBuf.writeUInt32BE(crc32(Buffer.concat([tBuf, chunk.data])));
    parts.push(lenBuf, tBuf, chunk.data, crcBuf);
  }
  return Buffer.concat(parts);
}

// 校验 PNG 结构完整性（签名 / CRC / IEND / chara 数据 / 尺寸体积），返回报告
function validatePngStructure(pngBuffer, expectedJson) {
  const checks = [];
  const push = (m) => checks.push(m);
  if (!pngBuffer || pngBuffer.length < 8 || pngBuffer.readUInt32BE(0) !== 0x89504e47) {
    push('❌ 非 PNG 文件，无法校验');
  ...
  while (offset + 12 <= pngBuffer.length) {
    const length = pngBuffer.readUInt32BE(offset);
    if (offset + 12 + length > pngBuffer.length) { push('❌ 数据块越界，文件可能被截断'); break; }
    const typeBuf = pngBuffer.subarray(offset + 4, offset + 8);
    const type = typeBuf.toString('latin1');
    const data = pngBuffer.subarray(offset + 8, offset + 8 + length);
    if (pngBuffer.readUInt32BE(offset + 8 + length) !== crc32(Buffer.concat([typeBuf, data]))) crcErrors++;

    if (firstChunk && type === 'IHDR' && length >= 8) {
    ...
    if (type === 'tEXt' || type === 'iTXt') {
      ...
      if (nullPos > 0) {
        ...
        if (kw === 'chara' || kw === 'ccv3') {
          charaFound = true;
          try {
            const parsed = readTavernPNGChunk(pngBuffer);
            const embeddedName = parsed && (parsed.data?.name || parsed.name);
            const expectedName = expectedJson && (expectedJson.data?.name || expectedJson.name);
            charaOk = !!embeddedName && (!expectedName || embeddedName === expectedName);
          } catch (e) { charaOk = false; }
        }
      }
    }
    offset += 12 + length;
  ...
  checks.push(crcErrors === 0 ? '✅ 全部数据块 CRC 正确' : `⚠ 发现 ${crcErrors} 个 CRC 错误（校准时将重算）`);
  checks.push(foundIEND ? '✅ IEND 结束符存在' : '⚠ IEND 缺失（校准时将补齐）');
  if (charaFound && charaOk) checks.push('✅ 内嵌角色数据完整可解析');
  else if (charaFound) checks.push('⚠ 内嵌角色数据异常（校准时将重新内嵌）');
  else checks.push('⚠ 未找到内嵌角色数据（校准时将重新内嵌）');
  if (width && height) {
    checks.push(`ℹ 图片尺寸 ${width}×${height}，文件 ${(pngBuffer.length / 1024 / 1024).toFixed(2)} MB`);
    if (width > 4096 || height > 4096) checks.push('⚠ 尺寸超过 4096px 上限（校准时将等比缩放）');
  }

  return { valid: crcErrors === 0 && foundIEND && charaFound && charaOk, checks };
}

// 把文件移入全局回收站（userData/jsTavern_Trash，绝不物理删除；EXDEV 降级为复制+删除）
async function trashFileToGlobalRecycle(filePath) {
  try {
...
app.whenReady().then(() => {
  ...
  // 🖼️ IPC：选择新封面图（原生文件选择器，选中目录加入白名单）
  ipcMain.handle('dialog:selectImageFile', async () => {
    ...
    if (canceled || filePaths.length === 0) return null;
    addAllowedRoot(path.dirname(filePaths[0])); // 选择目录加入白名单
    return filePaths[0];
  });

  // 🖼️ IPC：更换角色卡封面图（全格式：PNG/JSON/WebP 卡 → 统一生成 PNG 卡）
  ipcMain.handle('card:changeCover', async (event, { cardPath, newImagePath, cardData } = {}) => {
    try {
      if (!cardPath || typeof cardPath !== 'string' || !isPathAllowed(cardPath)) return forbidden();
      if (!newImagePath || typeof newImagePath !== 'string' || !isPathAllowed(newImagePath)) return forbidden();
      if (!fs.existsSync(cardPath)) return { success: false, error: '原卡文件不存在' };
      if (!fs.existsSync(newImagePath)) return { success: false, error: '新图文件不存在' };
      if (!cardData || typeof cardData !== 'object') return { success: false, error: '卡片数据为空' };

      const imageBuf = await fs.promises.readFile(newImagePath);
      let png = imageBufferToPng(imageBuf);
      if (!png) return { success: false, error: '无法解码新图（仅支持 PNG/JPEG/WebP）' };

      // 尺寸校准：超 4096px 等比缩放
      const sizeBefore = readPngDimensions(png);
      png = downscalePngIfNeeded(png, 4096);
      const sizeAfter = readPngDimensions(png);
      const scaleNote = (sizeBefore && sizeAfter &&
        (sizeBefore.width !== sizeAfter.width || sizeBefore.height !== sizeAfter.height))
        ? `（已从 ${sizeBefore.width}×${sizeBefore.height} 缩放）` : '';

      const out = embedCardJsonInPng(png, cardData, cardKeyword(cardData));
      if (!out) return { success: false, error: '内嵌卡片数据失败' };

      // 决定输出路径：原 PNG 覆盖；原 JSON/WebP 则转为同名 .png
      const oldExt = path.extname(cardPath).toLowerCase();
      let newFilePath = cardPath;
      if (oldExt !== '.png') {
        const base = path.basename(cardPath, oldExt) || 'character';
        newFilePath = path.join(path.dirname(cardPath), base + '.png');
        if (fs.existsSync(newFilePath)) {
          newFilePath = path.join(path.dirname(cardPath), `${base}_图_${Date.now()}.png`);
        }
      }
      if (!isPathAllowed(newFilePath)) return forbidden();

      // 换图前先备份旧版本到 .bak_history
      await processCardSnapshot(cardPath, false);
      await fs.promises.writeFile(newFilePath, out);

      // 原卡不是 PNG（JSON/WebP 转 PNG）时，把旧文件移入回收站，避免重复卡片
      let oldFilePath = null;
      if (path.resolve(newFilePath) !== path.resolve(cardPath)) {
        oldFilePath = cardPath;
        await trashFileToGlobalRecycle(cardPath);
      }

      const report = validatePngStructure(out, cardData);
      if (scaleNote) report.checks.push(`✅ 尺寸校准完成 ${scaleNote}`);

      return { success: true, newFilePath, fileName: path.basename(newFilePath), oldFilePath, report };
    } catch (e) {
      return { success: false, error: e.message };
    }
  });

  // 🔧 IPC：对当前角色卡做校验 + 自动校准（CRC/IEND/chara 数据/尺寸体积）
  ipcMain.handle('card:validate', async (event, { cardPath, cardData } = {}) => {
    try {
      if (!cardPath || typeof cardPath !== 'string' || !isPathAllowed(cardPath)) return forbidden();
      if (!fs.existsSync(cardPath)) return { success: false, error: '卡文件不存在' };
      const buf = await fs.promises.readFile(cardPath);
      if (sniffImageType(buf) !== 'png') {
        return { success: false, error: '仅支持对 PNG 角色卡进行校验校准（可先执行「换图」生成 PNG 卡）' };
      }

      const report = validatePngStructure(buf, cardData);
      let calibrated = false;
      if (!report.valid) {
        // 校准：用当前数据重新内嵌（重算 CRC、补齐 IEND、修复/重新写入 chara 数据），如需缩放再缩放
        await processCardSnapshot(cardPath, false);
        let png = imageBufferToPng(buf);
        png = downscalePngIfNeeded(png, 4096);
        const out = embedCardJsonInPng(png, cardData, cardKeyword(cardData));
        if (out) {
          await fs.promises.writeFile(cardPath, out);
          report.checks.push('✅ 已自动校准：重算 CRC / 补齐 IEND / 重新内嵌角色数据');
          calibrated = true;
        } else {
          report.checks.push('❌ 校准失败：无法重建 PNG');
        }
      } else {
        report.checks.push('✅ 卡片校验通过，无需校准');
      }

      return { success: true, report: { ...report, calibrated } };
    } catch (e) {
      return { success: false, error: e.message };
    }
  });
